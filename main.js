...
document.addEventListener('DOMContentLoaded', function() {
  ...
  class CardManager {
     ...
     */
    getRandomPosition() {
      // 保证可见：横向 12-78vw，纵向 20-82vh
      const left = Math.random() * 66 + 12; // 12vw - 78vw
      const top = Math.random() * 62 + 20;  // 20vh - 82vh
    ...
    startPopup() {
      ...
      const coverCount = Math.min(8, Math.max(6, Math.floor(this.blessings.length * 0.22)));
      ...
      // 等待开场覆盖后，再进入常规弹出
      const startDelay = coverCount ? coverCount * 80 + 240 : 0;
