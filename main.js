...
document.addEventListener('DOMContentLoaded', function() {
  ...
  class CardManager {
     ...
     */
    getRandomPosition() {
      // 确保卡片在可见区域内（留出边距，避免出屏）
      const left = Math.random() * 74 + 8; // 8% - 82%
      const top = Math.random() * 74 + 8;  // 8% - 82%
    ...
    /**
     * 生成居中附近的位置（用于开场快速遮挡彩蛋）
     */
    getCenteredPosition() {
      const left = 34 + Math.random() * 32;  // 34vw - 66vw，覆盖中心区域
      const top = 32 + Math.random() * 30;   // 32vh - 62vh
      const zIndex = 90 + Math.floor(Math.random() * 30);
      const rotation = (Math.random() - 0.5) * 14; // -7 到 7 度
      return { left, top, zIndex, rotation };
    }
    ...
    startPopup() {
      ...
      // 开场快速覆盖中心彩蛋：优先在中部投放 8-10 张
      const coverCount = Math.min(10, Math.max(8, Math.floor(this.blessings.length * 0.15)));
      for (let i = 0; i < coverCount && this.currentIndex < this.blessings.length; i++) {
        setTimeout(() => {
          const pos = this.getCenteredPosition();
          this.createCard(this.blessings[this.currentIndex], this.currentIndex, pos);
          this.currentIndex++;
        }, i * 80);
      }
      ...
      // 等待开场覆盖后，再进入常规弹出
      const startDelay = coverCount ? coverCount * 80 + 220 : 0;
      setTimeout(popNext, startDelay);
    }
